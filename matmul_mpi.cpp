...
/**
 * Classe pour la multiplication matrice-vecteur parallèle (MPI)
 */
class MatrixVectorMultiplierMPI {
private:
...
public:
     ...
     * Constructeur
     */
    MatrixVectorMultiplierMPI(int rows, int cols, int rank, int size) 
     ...
     * Note: chaque processus génère sa partie de la matrice
     */
    void initializeLocalMatrix(unsigned int seed = 42) {
        ...
        // Initialiser avec le même seed pour reproductibilité
        mt19937 gen(seed);
        uniform_real_distribution<double> dist(0.0, 1.0);
        
        // Sauter les valeurs des processus précédents
        for (int i = 0; i < start_row * cols_; i++) {
            dist(gen);
        }
        
        // Générer les valeurs pour ce processus
        for (int i = 0; i < local_rows_ * cols_; i++) {
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        cout << "=== Multiplication Matrice-Vecteur MPI (C++) ===" << endl;
        cout << "Taille de la matrice: " << rows << " x " << cols << endl;
        cout << "Nombre de processus MPI: " << size << endl;
    }
    
    // Création de l'objet
    MatrixVectorMultiplierMPI mvmul(rows, cols, rank, size);
